...
modified_indexers_vcheck=$(git diff --cached --diff-filter=M --name-only | grep ".yml" | grep "v[$min_schema-$max_schema]")
    ...
    for indexer in ${modified_indexers_vcheck}; do
        for ((i = max_schema; i >= min_schema; i--)); do
            ...
            if [ "$indexer_check" != "$indexer" ] && [ -f "$indexer_check" ]; then
                echo "--- Found [v$i] indexer for [$indexer] - backporting to [$indexer_check]"
                if $debug; then
...
newschema_indexers=$(git diff --cached --diff-filter=M --name-only | grep ".yml" | grep "v[$new_schema]")
